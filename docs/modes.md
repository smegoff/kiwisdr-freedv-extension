...
## Built-in reference tests

The receiver test button follows the selected codec:

- **Test RADE** is shown when RADEV1 is selected. It sends a deterministic
  RADEV1 waveform generated by the pinned portable C `rade_tx_wav` utility.
- **Test 700D** is shown for every legacy selection. It sends the established
  700D Codec2 reference so results remain comparable between releases.

Both references use the same BSD-licensed speech source and traverse the real
Kiwi sound callback, authenticated decoder camper, modem backend, paced return
queue and normal browser audio path. Test sessions are excluded from FreeDV
Reporter.

700D speech is expected to sound narrow and synthetic because the 700C speech
codec runs at roughly 700 bit/s. Its strong LDPC modem protection improves
weak-signal recovery, not speech fidelity. RADEV1 uses neural speech features
and FARGAN synthesis, so its clean reference should sound noticeably more
natural. Neither reference should have repeated periodic gaps or transport
stutter when the system is healthy.
