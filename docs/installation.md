...
written for Kiwi extension `0.1.34`, decoder service `0.1.26`, and KiwiSDR
...
Allow TCP 8076 only from the configured management CIDR. There is no dashboard
login, so every host admitted by that rule can view diagnostics. Do not expose
...
Before the modem tests, enter `14.236 MHz` in **Manual frequency**, press
**Tune**, and require the receiver to tune 14.236 MHz USB without using the
Kiwi's native frequency field. Confirm equivalent kHz and Hz input. Require the
calling-frequency selector to retain its static entries; any currently
advertised frequencies must be labelled **[Reporter live]** and must disappear
after the advertising station disconnects or the 120-second authenticated
cache expires.
