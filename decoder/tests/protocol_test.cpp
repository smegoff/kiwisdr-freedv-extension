...
int main() {
  ...
  const std::string raw = R"({"protocol":2,"generation":9,"running":true,"rx_chan":3,"mode":"700D","input_rate":12000,"frequency_hz":7177000})";
  const auto job = kfd::parse_decoder_job(kfd::url_encode(raw));
  assert(job.running && job.generation == 9 && job.rx_channel == 3 && job.mode == "700D");
